...
namespace Tellurian.Trains.Repositories.Xpln
{
    ...
    public sealed class XplnRepositoryWithExcel : ILayoutReadStore, ITimetableReadStore, IScheduleReadStore, IDisposable
    {
        ...
        public XplnRepositoryWithExcel(DirectoryInfo documentsDirectory)
        {
        ...
        public RepositoryResult<Layout> GetLayout(string name)
        {
            ...
            try
            {
                ...
                if (messages.HasNoStoppingErrors()) messages.AddRange(GetStretches(result, book));
            }
            finally
            {
        ...
        private static IEnumerable<Message> GetTrains(Timetable timetable, Excel.Workbook book)
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    {
                        case "TRAINDEF":
                            if (currentTrain != null) timetable.Add(currentTrain);
                ...
                else
                {
                    ...
                    switch (type)
                    {
                        ...
                        case "LOCOMOTIVE":
                            ...
                            if (!string.IsNullOrEmpty(locoId))
                            {
                                ...
                                if (currentTrain != null)
                                {
                                    ...
                                    if (fromCall.IsNone) locoMessages.Add(Message.Error(string.Format(CultureInfo.CurrentCulture, Resources.Strings.ObjectAtStationWithDepartureDoNotRefersToAnExistingTimeInTrain, locoId, fromStationSignature, fromTime, currentTrain)));
                                    if (toCall.IsNone) locoMessages.Add(Message.Error(string.Format(CultureInfo.CurrentCulture, Resources.Strings.ObjectAtStationWithArrivalDoNotRefersToAnExistingTimeInTrain, locoId, fromStationSignature, toTime, currentTrain)));
                                    if (fromIndex >= toIndex) locoMessages.Add(Message.Error(string.Format(CultureInfo.CurrentCulture, Resources.Strings.ObjectInTrainHasWrongTimingEndStartionIsBeforeStartStation, locoId, currentTrain, fromTime, toTime)));
                        ...
                        case "TRAINSET":
                            ...
                            {
                                if (currentTrain.Number == "2003") Debugger.Break();
                                var fromStationSignature = row.Value(3);
                                var toStationSignature = row.Value(4);
                                var fromTime = row.Value(5).AsTime();
                                var toTime = row.Value(6).AsTime();
                                var (fromCall, fromIndex) = currentTrain.FindBetweenArrivalAndDeparture(fromStationSignature, fromTime);
                                var (toCall, toIndex) = currentTrain.FindBetweenArrivalAndDeparture(toStationSignature, toTime);
                                if (fromCall.HasValue && toCall.HasValue)
                                {
                                ...
                                {
                                    throw new ArgumentOutOfRangeException($"Trainset {trainsetId} in {currentTrain.Number} between {fromStationSignature} {fromTime} and {toStationSignature} {toTime} has missing calls.");
                                }
                            }
                            break;
                        case "JOB":
                            ...
                            if (!string.IsNullOrEmpty(jobId))
                            {
                                ...
                                if (jobMessages.HasNoStoppingErrors())
                                {
